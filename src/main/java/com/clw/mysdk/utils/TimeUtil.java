...
public abstract class TimeUtil {
	...
	/**
	 * 日期格式1（yyyyMMddHHmmss），精确到秒
	 */
	private static SimpleDateFormat mSimpleDateFormat_1=new SimpleDateFormat("yyyyMMddHHmmss");
	
	/**
	 * 日期格式2（yyyyMMddHHmmssS），精确到毫秒
	 */
	private static SimpleDateFormat mSimpleDateFormat_2=new SimpleDateFormat("yyyyMMddHHmmssS");
	
	/**
	 * 日期格式3（yyyy-MM-dd HH:mm:ss），精确到秒
	 */
	private static SimpleDateFormat mSimpleDateFormat_3 = new SimpleDateFormat("yyyy-MM-dd HH:mm:ss");
	...
	public static String getCurrentTime_1(){
		...
		try {
			currentTime=mSimpleDateFormat_1.format(new Date());
		} catch (Exception e) {
	...
	/**
	 * 获取当前时间，精确到毫秒（格式：yyyyMMddHHmmssS）
	 * @return 当前时间字符串（格式：yyyyMMddHHmmssS）
	 */
	public static String getCurrentTime_2(){
		String currentTime=null;
		try {
			currentTime=mSimpleDateFormat_2.format(new Date());
		} catch (Exception e) {
			Log.e(TAG, e.getMessage());
		}
		return currentTime;
	...
	/**
	 * 获取当前时间，精确到毫秒（格式：yyyy-MM-dd HH:mm:ss）
	 * @return 当前时间字符串（格式：yyyy-MM-dd HH:mm:ss）
	 */
	public static String getCurrentTime_3(){
		String currentTime=null;
		try {
			currentTime=mSimpleDateFormat_3.format(new Date());
		} catch (Exception e) {
			Log.e(TAG, e.getMessage());
		}
		return currentTime;
	...
	/**
	 * 获取智能日期
	 * @param timeString 日期字符串（格式：yyyyMMddHHmmss），精确到秒
	 * @return
	 */
	public static String getSmartDate_1(String timeString){
		Date date=null;
		try {
			date=mSimpleDateFormat_1.parse(timeString);
		} catch (Exception e) {
			Log.e(TAG, e.getMessage());
		}
	...
	/**
	 * 获取智能日期
	 * @param timeString 日期字符串（格式：yyyyMMddHHmmssS），精确到毫秒
	 * @return
	 */
	public static String getSmartDate_2(String timeString){
		Date date=null;
		try {
			date=mSimpleDateFormat_2.parse(timeString);
		} catch (Exception e) {
			Log.e(TAG, e.getMessage());
		}
		return getSmartDate(date);
	...
	/**
	 * 获取智能日期
	 * @param timeString 日期字符串（格式：yyyy-MM-dd HH:mm:ss），精确到秒
	 * @return
	 */
	public static String getSmartDate_3(String timeString){
		Date date=null;
		try {
			date=mSimpleDateFormat_3.parse(timeString);
		} catch (Exception e) {
			Log.e(TAG, e.getMessage());
		}
		return getSmartDate(date);
	 ...
	 * @return
	 */
	private static String getSmartDate(Date date) {
	 ...
	 */
	@SuppressLint("SimpleDateFormat")
	private static String getSmartDate(long date) {
		...
		String time = " " + new SimpleDateFormat("HH:mm").format(date).toString();
		...
		if (nowDetails[0] == smartDetail[0]) {		//this year
			if(nowDetails[1] == smartDetail[1]) {	//this month
				...
				if (day >= 3) {			//fomer day
					smartDate = String.valueOf(smartDetail[1]) + "月"+String.valueOf(smartDetail[2]) + "日" + time;
	 ...
	 * @return
	 */
	protected static String getSmartTime(long duration) {
